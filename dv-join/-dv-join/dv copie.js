...
/*	Paramètres SVG	**** */
		...
		svg = {
			...
			_svg1: { // pré-définitions dans un objet provisoire - de propriétés propres à l'instance
						/* to do : d'où une bibliothèque de manipulation pour plusieurs dv : OK ? */
				get echelles () {
...
// "join"
		var joinActu = {
				get projet () { // évolution version 4 : update permanent
					delete this.projet;
					return this.projet = joinPerm.cadastre.data(d3.range(datail.historique[0].aPourvoir));
				},
				get lotissement () { // semblable version 3
					delete this.lotissement;
					return this.lotissement = this.projet.enter();
				},
				get travauxLotissement () { // semblable version 3
					delete this.travauxLotissement;
					return this.travauxLotissement = this.lotissement.append("li");
				},
				get travauxGeneraux () { // nouveauté version 4, qui assure l'indépendance de l'update
					delete this.travauxGeneraux;
					return this.travauxGeneraux = this.travauxLotissement.merge(joinActu.projet); // qu'importe ordre
				},
				get declassement () { // semblable version 3
					delete this.declassement;
					return this.declassement = this.projet.exit();
				},
				get indiceConserv () {
...
// dessin des cercles, pour aPourvoir
		joinActu.cercles.projet = dv.dv1.baseCercles
		.selectAll("circle")
		.data(datail.historique);
		joinActu.cercles.travauxLotissement = joinActu.cercles.projet.enter()
			.append("circle")
			.attr("r", 7);
		joinActu.cercles.projet.merge(joinActu.cercles.travauxLotissement)
